...
import { Shield, Eye, CheckCircle, ArrowRight, Scan, Lock, Zap } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const createFaceGeometry = () => {
      ...
      const vertices = new Float32Array([
        // Face outline
        0, 1.2, 0,      // Top of head
        -0.8, 0.8, 0.3,  // Left temple
        0.8, 0.8, 0.3,   // Right temple
        -0.6, 0.4, 0.5,  // Left cheek upper
        0.6, 0.4, 0.5,   // Right cheek upper
        -0.7, 0, 0.4,    // Left cheek mid
        0.7, 0, 0.4,     // Right cheek mid
        -0.5, -0.4, 0.3, // Left jaw
        0.5, -0.4, 0.3,  // Right jaw
        0, -0.8, 0.2,    // Chin
        
        // Eye area
        -0.3, 0.2, 0.6,  // Left eye inner
        -0.15, 0.25, 0.65, // Left eye top
        -0.45, 0.15, 0.55, // Left eye outer
        0.3, 0.2, 0.6,   // Right eye inner
        0.15, 0.25, 0.65, // Right eye top
        0.45, 0.15, 0.55, // Right eye outer
        
        // Nose
        0, 0.1, 0.7,     // Nose bridge
        -0.1, -0.1, 0.6, // Left nostril
        0.1, -0.1, 0.6,  // Right nostril
        0, -0.15, 0.55,  // Nose tip
        
        // Mouth
        -0.2, -0.35, 0.4, // Left mouth corner
        0, -0.3, 0.45,    // Mouth center
        0.2, -0.35, 0.4,  // Right mouth corner
      ]);
    ...
    const leftIris = new THREE.Mesh(new THREE.SphereGeometry(0.04, 12, 8), irisMaterial);
    leftIris.position.set(-0.3, 0.2, 0.68);
    faceGroup.add(leftIris);
    
    const leftPupil = new THREE.Mesh(new THREE.SphereGeometry(0.02, 8, 6), pupilMaterial);
    ...
    const rightIris = new THREE.Mesh(new THREE.SphereGeometry(0.04, 12, 8), irisMaterial);
    rightIris.position.set(0.3, 0.2, 0.68);
    faceGroup.add(rightIris);
    
    const rightPupil = new THREE.Mesh(new THREE.SphereGeometry(0.02, 8, 6), pupilMaterial);
    ...
    camera.position.z = 4;
    sceneRef.current = { scene, camera, renderer, faceGroup, scanRing, particles };
  ...
  const features = [
    {
      ...
      description: 'Multi-layer face liveness detection prevents spoofing attacks using photos, videos, or masks.',
      gradient: 'from-blue-500 to-purple-600'
    },
    {
      icon: <Scan className="w-8 h-8" />,
      title: 'Real-time Detection',
      description: 'Instant face detection and liveness verification with sub-500ms response time.',
      gradient: 'from-green-500 to-teal-600'
    },
    {
      icon: <Lock className="w-8 h-8" />,
      title: 'UIDAI Compliant',
      description: 'Fully compliant with UIDAI security standards and guidelines for biometric authentication.',
  ...
  const steps = [
    {
      ...
      description: 'Provide your 12-digit Aadhaar number to start the authentication process.',
      icon: <Eye className="w-6 h-6" />
    },
    {
      number: '02',
      title: 'Liveness Detection',
      description: 'Follow on-screen instructions for eye blink, smile, or head movement detection.',
      icon: <Scan className="w-6 h-6" />
    },
    {
      number: '03',
      title: 'Face Authentication',
      description: 'Your face is verified against secure biometric data for final authentication.',
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2 px-3 sm:px-4 py-2 bg-green-50 rounded-full">
              ...
              <span className="text-xs sm:text-sm font-medium text-green-700 whitespace-nowrap">UIDAI Certified</span>
            </div>
          </div>
        </div>
      </header>
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            ...
            <div className="space-y-6 lg:space-y-8 text-center lg:text-left">
              <div className="inline-flex items-center space-x-2 px-3 sm:px-4 py-2 bg-blue-50 rounded-full mx-auto lg:mx-0">
                ...
                <span className="text-xs sm:text-sm font-medium text-blue-700">Next-Gen Biometric Authentication</span>
              </div>
              ...
              <p className="text-base sm:text-lg lg:text-xl text-gray-600 leading-relaxed max-w-2xl mx-auto lg:mx-0">
                Revolutionary biometric authentication powered by advanced AI. Experience 
                military-grade security with seamless user experience in milliseconds.
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                ...
                <button className="px-6 sm:px-8 py-3 sm:py-4 bg-white border-2 border-gray-200 text-gray-700 rounded-xl sm:rounded-2xl font-semibold text-base sm:text-lg hover:border-gray-300 hover:shadow-lg transform hover:-translate-y-1 transition-all duration-300 min-h-[52px] flex items-center justify-center">
              ...
              <div className="grid grid-cols-3 gap-4 sm:gap-8 pt-6 lg:pt-8 max-w-md mx-auto lg:max-w-none lg:mx-0">
                <div className="text-center">
                  <div className="text-2xl sm:text-3xl font-bold text-gray-800">99.9%</div>
                  <div className="text-xs sm:text-sm text-gray-600">Accuracy Rate</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl sm:text-3xl font-bold text-gray-800">&lt;500ms</div>
                  <div className="text-xs sm:text-sm text-gray-600">Response Time</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl sm:text-3xl font-bold text-gray-800">10M+</div>
                  <div className="text-xs sm:text-sm text-gray-600">Authentications</div>
                </div>
              </div>
            </div>
            ...
            <div className="relative flex justify-center lg:justify-end">
              <div className="relative bg-gradient-to-br from-white to-gray-50 rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 shadow-2xl w-full max-w-md lg:max-w-lg">
                ...
                <div ref={mountRef} className="relative z-10 flex justify-center items-center h-64 sm:h-80"></div>
                ...
                <div className="absolute top-2 sm:top-4 right-2 sm:right-4 flex items-center space-x-2 px-2 sm:px-3 py-1 sm:py-2 bg-green-100 rounded-full">
                  ...
                  <span className="text-xs sm:text-sm font-medium text-green-700">Live Detection</span>
                </div>
                  ...
                  <div className="bg-white/80 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2 sm:space-x-3">
                        ...
                        <span className="text-xs sm:text-sm font-medium text-gray-700">Scanning biometric data...</span>
                      </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12 lg:mb-16">
            ...
            <p className="text-lg sm:text-xl text-gray-600 max-w-3xl mx-auto">
              Experience the future of biometric authentication with our cutting-edge technology
            </p>
          </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
            {features.map((feature, index) => (
              <div key={index} className="group relative">
                <div className="absolute inset-0 bg-gradient-to-r opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl sm:rounded-3xl blur-xl" 
                ...
                <div className="relative bg-white border border-gray-100 rounded-2xl sm:rounded-3xl p-6 lg:p-8 shadow-lg hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300">
                  <div className={`inline-flex p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-gradient-to-r ${feature.gradient} text-white mb-4 sm:mb-6`}>
                    {feature.icon}
                  </div>
              ...
              <div key={index} className="relative">
                <div className="bg-white rounded-2xl sm:rounded-3xl p-6 lg:p-8 shadow-lg hover:shadow-2xl transform hover:-translate-y-2 transition-all duration-300">
                  <div className="flex items-center justify-between mb-4 sm:mb-6">
                    <div className="text-4xl sm:text-6xl font-bold text-gray-200">{step.number}</div>
                    <div className="p-2 sm:p-3 bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl sm:rounded-2xl text-white">
                      {step.icon}
                    </div>
                  </div>
                  <h3 className="text-xl sm:text-2xl font-bold text-gray-800 mb-3 sm:mb-4">{step.title}</h3>
      ...
      <section className="py-12 sm:py-16 lg:py-20 bg-gradient-to-r from-blue-600 via-purple-600 to-green-600 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-lg sm:text-xl text-blue-100 mb-8 sm:mb-10 leading-relaxed">
            Join millions who trust Xyndrix for secure, seamless biometric authentication
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center">
            ...
            <button className="px-8 sm:px-10 py-4 sm:py-5 border-2 border-white text-white rounded-xl sm:rounded-2xl font-bold text-base sm:text-lg hover:bg-white hover:text-gray-800 transform hover:-translate-y-1 transition-all duration-300 min-h-[56px] flex items-center justify-center">
              Learn More
            </button>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-center space-x-3 mb-6 sm:mb-8">
            <div className="w-10 sm:w-12 h-10 sm:h-12 bg-gradient-to-r from-red-500 to-red-600 rounded-lg flex items-center justify-center transform rotate-12">
              <span className="text-white font-bold text-xl sm:text-2xl transform -rotate-12">X</span>
            </div>
            <span className="text-2xl sm:text-3xl font-bold">Xyndrix</span>
          </div>
          <p className="text-center text-gray-400 text-sm sm:text-base">
            © 2024 Xyndrix. Securing the future with advanced biometric technology.
          </p>
        </div>
      </footer>
    </div>
